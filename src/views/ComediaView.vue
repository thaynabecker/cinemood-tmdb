...
const comedyMovies = ref([])        // alterado de romanceMovies
...
onMounted(async () => {
  try {
    ...
    comedyMovies.value = response.data.results.filter(
      (m) => m.poster_path
    );

    featuredMovie.value = comedyMovies.value[0];
...
<template>
  ...
  <section class="comedia">
    ...
    <!-- TITULO -->
    <h1 class="page-title">Comédia</h1>
    ...
    <main v-if="featuredMovie" class="featured">
      ...
      <div class="featured-info">
        ...
        <p class="featured-genre">Gêneros: Comédia</p>
      </div>
    </main>
...
.details { 
  ...
  background-color: #da9800; margin: 2vw; 
  border-radius: 2vw;
 } 
