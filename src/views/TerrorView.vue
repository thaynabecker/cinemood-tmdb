...
onMounted(async () => {
  try {
    ...
    terrorMovies.value = response.data.results.filter(
      (m) => m.poster_path
    );
...
<template>
  ...
  <section class="terror">
    ...
    <img src="/img/terrormao.png" alt="Mão de sangue" class="blood-hand">
    <!-- BANNER PRINCIPAL -->
    <main v-if="featuredMovie" class="featured">
      ...
      <div class="featured-info">
        ...
        <p class="featured-genre">Gêneros: Terror</p>
      </div>
    </main>
...
.blood-hand {
  position: absolute;
  top: -6px;       /* ajusta para pegar parte do banner */
  right: 110px;         /* lado direito */
  width: 300px;     /* ajusta o tamanho */
  z-index: 3;       /* acima do banner e dos detalhes */
  pointer-events: none; /* para não atrapalhar clique nos filmes */
}
...
.details { 
  ...
  background-color: #8a0e0e; margin: 2vw; 
  border-radius: 2vw;
 } 
